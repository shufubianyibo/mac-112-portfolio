...
It works, but both dataset have a column names "grade" but they are different variable, but the join function combine them and only keep one of them.
...
'grade.x' and 'grand.y' are the variable named grade from the two datasets.
...
courses_combined <- courses |> 
  group_by(sessionID) |> 
  mutate(enroll = sum(enroll)) |>
  distinct(sessionID, enroll)
...
student_class_size <- grades%>%
  left_join(courses_combined, by = c("sessionID" = "sessionID"))%>%
  group_by(sid)%>%
  mutate(median = median(enroll))
...
```{r}
grades_E <- grades%>%
  semi_join(dept_E, by = c("sessionID" = "sessionID"))
grades_E
```
...
```{r}
course_dept <- courses%>%
  left_join(grades,  by = c("sessionID" = "sessionID"))%>%
  group_by(dept)
```
