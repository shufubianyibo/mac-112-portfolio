...
The units of observation is the reaction time every day of each participants.
...
`ggplot(sleep_wide, aes(y = time, x = day, color = subject))`

But we don't have a column for day.
...
By using 'pivot_longer()' to generate all 'day_0' to 'day_9' to one column.
...
Mostly, the more days the participants get only 3 hours sleep, the longer their reaction time is. It isn't same for everybody, some people's reaction time increases faster, and some people slower, and some even decreases.
